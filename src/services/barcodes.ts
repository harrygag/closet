...
/**
 * Generates unit-specific barcodes for multi-quantity items
 * Format: Base barcode + unit suffix (e.g., INV-20241118-XXX-12345-U01)
 *
 * @param baseBarcode - The original item barcode
 * @param quantity - Number of units to generate
 * @returns Array of unit-specific barcodes
 */
export function generateUnitBarcodes(baseBarcode: string, quantity: number): string[] {
  if (quantity <= 1) {
    return [baseBarcode];
  }
  ...
  for (let i = 1; i <= quantity; i++) {
    const unitNum = i.toString().padStart(2, '0'); // U01, U02, etc.
    barcodes.push(`${baseBarcode}-U${unitNum}`);
  }
  return barcodes;
}

/**
 * Parses a unit barcode to extract the base barcode and unit number
 * Format: INV-20241118-XXX-12345-U01
 *
 * @param unitBarcode - Unit-specific barcode
 * @returns Object with baseBarcode and unitNumber, or null if not a unit barcode
 */
export function parseUnitBarcode(unitBarcode: string): { baseBarcode: string; unitNumber: number } | null {
  const match = unitBarcode.match(/^(.+)-U(\d{2})$/);
  if (!match) {
    // Not a unit barcode, return as base barcode with unit 1
    return { baseBarcode: unitBarcode, unitNumber: 1 };
  }
