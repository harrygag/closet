...
export const database = {
  ...
  from: (tableName: string) => {
    return {
      ...
      update: (data: any) => {
        const constraints: QueryConstraint[] = [];
        let directDocId: string | null = null;

        const builder = {
          eq: (field: string, value: any) => {
            if (field === 'id') {
              // Firestore document IDs are not fields — store separately for direct ref lookup
              directDocId = value;
            } else {
              constraints.push(where(field, '==', value));
            }
          ...
          select: () => {
            return {
              single: async () => {
                try {
                  if (directDocId) {
                    // Fast path: update by document ID directly
                    const docRef = doc(db, tableName, directDocId);
          ...
          then: (resolve: any) => {
            (async () => {
              try {
                if (directDocId) {
                  // Fast path: update by document ID directly
                  await updateDoc(doc(db, tableName, directDocId), {
                    ...data,
                    updated_at: serverTimestamp()
                  });
                } else {
        ...
        const builder = {
          ...
          then: (resolve: any) => {
            (async () => {
              try {
                if (directDocId) {
                  // Fast path: delete by document ID directly (Firestore rules enforce ownership)
                  await deleteDoc(doc(db, tableName, directDocId));
                } else {
                  const q = query(collection(db, tableName), ...constraints);
                  const snapshot = await getDocs(q);
                  for (const docSnap of snapshot.docs) {
