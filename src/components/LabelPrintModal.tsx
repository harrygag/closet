...
export const LabelPrintModal = ({ item, open, onClose, userId }: LabelPrintModalProps) => {
  ...
  // Determine what content to encode in the QR code
  const qrContent = useMemo(() => {
    if (!item) return null;
    return `${window.location.origin}/items/${item.id}`;
  }, [item?.id]);

  // Generate QR code as a data URL using the qrcode package
  useEffect(() => {
    if (!qrContent) {
      setQrDataUrl(null);
      return;
    ...
    let cancelled = false;
    QRCode.toDataURL(qrContent, { width: 150, margin: 1 })
      .then((url) => {
        ...
        <div ref={labelRef} className="rounded-lg border border-gray-600 bg-white p-4 text-black">
            ...
            <>
              <div
                style={{
                  ...
                  textAlign: 'center',
                  color: '#000',
                  fontSize: preset.widthIn === 1 ? '7px' : '9px',
                }}
              >
                RETROTHRIFTCO
              </div>
          ...
          <div className="flex justify-between text-xs font-semibold">
            ...
            <span style={{ fontWeight: 700 }}>
              Size {displaySize(item.size) || '—'} • Hanger {item.hangerId || '—'}
            </span>
          </div>
