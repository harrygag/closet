/**
 * Whatnot Import Modal — same shape as FacebookImportModal, driven by
 * `marketplaceData/whatnot_{uid|me}` documents written by the (pending) Whatnot
 * content script. Opens the Whatnot seller listings page and loads whatever the
 * extension wrote.
 */
...
import { WhatnotListing, importWhatnotItems, clearStaleWhatnotBindings } from '../../services/whatnot/import';
...
const WN_SELLING_URL = 'https://www.whatnot.com/selling/listings#autoScroll';

interface WhatnotImportModalProps {
  open: boolean;
  onClose: () => void;
}

function listingKey(l: WhatnotListing): string {
  return l.listing_id || l.id;
}

export const WhatnotImportModal = ({ open, onClose }: WhatnotImportModalProps) => {
    ...
    try {
      ...
      for (const docId of candidateDocIds) {
        try {
          ...
          if (snap.exists()) {
            const d = snap.data();
            if ((!d.platform || d.platform === 'whatnot') && Array.isArray(d.listings) && d.listings.length > 0) {
              data = d;
              break;
            }
          }
      ...
      if (!data) {
        setStatusMessage('No Whatnot listings found yet. The extension scrape may still be loading.');
        return;
      }
      setListings(data.listings as WhatnotListing[]);
    } catch (err) {
      console.error('[WhatnotImportModal] Load error:', err);
      setStatusMessage('Failed to load Whatnot listings');
    }
  }, [user]);
  ...
  const startCapture = useCallback(async () => {
    ...
    setListings([]);
    setSelection({});
    setStatusMessage('Opening Whatnot seller listings — leave the tab open while it scrolls.');
    setIsCapturing(true);
    try { await deleteDoc(doc(db, 'marketplaceData', `whatnot_${user.id}`)); } catch { /* not present yet */ }
    window.open(WN_SELLING_URL, '_blank');
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const result = await importWhatnotItems(toImport, user.id, matchMap);
      toast.success(`Linked ${result.imported} listings (${result.unmatched} orphan)`);
      // Auto-clear stale bindings against the FULL captured set.
      try {
        const cleared = await clearStaleWhatnotBindings(user.id, listings.map((l) => listingKey(l)).filter(Boolean));
        if (cleared > 0) toast.info(`${cleared} item${cleared === 1 ? '' : 's'} no longer on Whatnot — moved to should-list`);
      } catch (e) { console.error('[WhatnotImportModal] stale-binding cleanup failed', e); }
  ...
  return (
    <Modal open={open} onOpenChange={onClose} title="Import Whatnot Listings" size="xl">
      <div className="flex flex-col h-[80vh]">
        <div className="flex-shrink-0 pb-4 border-b border-gray-700">
            ...
            <div className="flex items-center gap-3">
              {isCapturing ? (
                <div className="flex items-center gap-2">
                  <Loader2 className="h-5 w-5 animate-spin" style={{ color: WN_YELLOW }} />
                  <span className="font-medium" style={{ color: WN_YELLOW }}>
                ...
                <span className="text-gray-400 text-sm">{statusMessage || 'Awaiting Whatnot scrape…'}</span>
              )}
            </div>
            <div className="flex items-center gap-2">
              <Button size="sm" variant="ghost" onClick={() => { window.open(WN_SELLING_URL, '_blank'); setTimeout(() => window.focus(), 500); }}>
                <ExternalLink className="h-4 w-4 mr-1" /> Reopen Whatnot
              </Button>
              <Button size="sm" variant="ghost" onClick={loadListings}>
                <RefreshCw className="h-4 w-4" />
              </Button>
              <Button size="sm" onClick={startCapture} style={{ backgroundColor: WN_YELLOW, color: '#1A1A1A' }}>
                Retry Capture
              </Button>
            </div>
          </div>
          <div className="flex items-center justify-between">
            ...
            <Button onClick={handleImport} disabled={importing || totalSelected === 0} style={{ backgroundColor: WN_YELLOW, color: '#1A1A1A' }}>
        ...
        <div className="flex-1 overflow-y-auto py-4">
          {listings.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-64 text-gray-400">
              ...
              <p className="text-lg">No Whatnot listings captured yet</p>
              <p className="text-sm mt-2 max-w-md text-center">
                The extension scrape for Whatnot is being built. For now the seller listings page
                        ...
                        {cands.length === 0 ? (
                          ...
                          <button
                            ...
                            className={`flex items-center gap-1.5 px-2 py-1 rounded border text-[11px] ${
                              sel === c.ebayItem.id
                                ? 'border-yellow-400/70 bg-yellow-900/30 text-yellow-100'
