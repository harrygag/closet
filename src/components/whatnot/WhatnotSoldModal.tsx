/**
 * Whatnot Sold Modal — mirrors FacebookSoldModal. Reads
 * `marketplaceData/whatnot_sold_*` (written by the pending Whatnot content
 * script). Sold Whatnot items, once matched, decrement real stock via a
 * unitSales entry platform='whatnot'.
 */
...
import type { WhatnotListing } from '../../services/whatnot/import';

const db = getFirestore(app);
const WN_YELLOW = '#FFDE2D';
const WN_SOLD_URL = 'https://www.whatnot.com/selling/sold#autoScroll';

interface WhatnotSoldModalProps {
  open: boolean;
  onClose: () => void;
}

export const WhatnotSoldModal = ({ open, onClose }: WhatnotSoldModalProps) => {
  ...
  const loadListings = useCallback(async () => {
    ...
    setStatusMessage('');
    try {
      const candidateDocIds = [`whatnot_sold_${user.id}`, 'whatnot_sold_me'];
      let data: any = null;
      ...
      if (!data) {
        try {
          ...
          for (const ds of all.docs) {
            if (!ds.id.startsWith('whatnot_sold_')) continue;
            const d = ds.data();
            if (Array.isArray(d.listings) && d.listings.length > 0) { data = d; break; }
          }
        } catch {}
      ...
      if (!data) {
        setStatusMessage('No Whatnot sold items found yet. The extension scrape will populate this once wired.');
        return;
      }
  ...
  const startCapture = useCallback(async () => {
    ...
    setIsCapturing(true);
    setStatusMessage('Opening Whatnot sold listings — leave the tab open while it scrolls.');
    window.open(WN_SOLD_URL, '_blank');
  ...
  return (
    <Modal open={open} onOpenChange={onClose} title="Sold Whatnot Items" size="xl">
      <div className="flex flex-col h-[80vh]">
        <div className="flex-shrink-0 pb-4 border-b border-gray-700 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Button size="sm" variant="ghost" onClick={() => { window.open(WN_SOLD_URL, '_blank'); setTimeout(() => window.focus(), 500); }}>
        ...
        <div className="flex-1 overflow-y-auto py-4">
          {soldListings.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-64 text-gray-400">
              ...
              <p className="text-lg">No sold Whatnot items found</p>
              <p className="text-sm mt-2 max-w-md text-center">
                When the extension scrape lands, your Whatnot sold listings will surface here so they
                can be matched to inventory and decrement eBay stock.
              </p>
            </div>
          ) : (
