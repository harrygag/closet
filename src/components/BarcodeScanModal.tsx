...
import { logScan, logMarkSold } from '../services/activityLog';
...
export const BarcodeScanModal: React.FC<BarcodeScanModalProps> = ({
  ...
  const lastScannedRef = useRef<{ barcode: string; ts: number }>({ barcode: '', ts: 0 });
  ...
  const handleBarcodeScanned = (barcode: string) => {
    ...
    const now = Date.now();
    if (barcode === lastScannedRef.current.barcode && now - lastScannedRef.current.ts < 3000) return;
    lastScannedRef.current = { barcode, ts: now };
    ...
    if (item) {
      ...
      // Keep camera running — don't reset codeReader
      ...
      if (user) {
        logScan(
          ...
          item.ebayUrl ?? undefined,
          item.ebayListingId ?? undefined
  ...
  const handleMarkAsSold = () => {
    if (foundItem) {
      ...
      // Log the mark-as-sold activity
      if (user) {
        logMarkSold(
          user.id,
          foundItem.id,
          foundItem.name,
          Math.round((foundItem.sellingPrice ?? 0) * 100),
          foundItem.ebayUrl ?? undefined
        ).catch((err) => console.error('Failed to log mark sold:', err));
      }
  ...
  const resetScanning = () => {
    ...
    setError('');
    setManualInput('');
    lastScannedRef.current = { barcode: '', ts: 0 };
    // Camera stays running — no restart needed
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
      <div className="relative w-full max-w-2xl rounded-lg bg-gray-800 p-6 shadow-xl mx-4">
        ...
        {/* Camera View — always in DOM when camera mode, hidden when result showing */}
        {isScanning && !useManualMode && (
          <div className={`mb-6 ${foundItem ? 'hidden' : ''}`}>
