/**
 * Import Vendoo CSV in small batches via Supabase MCP
 */
...
function mapVendooRowToItem(row, userId) {
  ...
  const description = (row['Description'] || '').substring(0, 500);
...
async function main() {
  console.log('📋 Vendoo CSV Batch Import');
  console.log('================================\n');
  ...
  if (authError) {
    ...
    console.log('\n⚠️  Please run this after signing in through the app,');
    console.log('    or update the email/password in the script.');
    process.exit(1);
  }
  ...
  let successCount = 0;
  const batchSize = 10; // Small batches for RLS
